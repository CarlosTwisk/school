...
fig = px.bar(Athletes['NOC'].value_counts(),
...
fig = px.bar(Athletes['Discipline'].value_counts(), labels={'index':'Discipline', 'value':'Athlete count'}, title='Amount of athletes per discipline')
fig.show()
...
st.header('Fails form this project')


# In[43]:


#df_groupby = px.data.gapminder().query(df_groupby['NOC'])
#Athletes['NOC'] = px.data.gapminder().query('NOC')
#Athletes.loc[Athletes['NOC'].value_counts() < 50] = 'Other countries' # Represent only large countries
#Athletes['NOC'] = np.where(Athletes['NOC'].value_counts()<50, 'Other countries','NOC')
fail = px.pie(Athletes, values=Athletes['NOC'].value_counts(), title='Deelnemende landen')

check_3= st.checkbox('Show fail')
